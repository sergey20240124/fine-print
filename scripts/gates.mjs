...
/* Ворота — жёсткие условия. Провал любого означает отказ.
   Отбирались по калибровке: критерий, который отсекает почти все заведомо
   разрешимые рынки, не несёт информации и воротами быть не может. */
...
/* Атрибуты — измеряются и показываются, но не отсеивают.
   threshold и missing понижены из ворот: на выборке чисто разрешившихся
   рынков они отсекали 96,8% и 100% соответственно. Требование числа
   исключало событийные контракты, а оговорка про отсутствие данных нужна
   только рынкам, привязанным к публикуемому ряду. Вопрос «определяется ли
   исход механически» решает стадия C, где он и решается по существу.
   dates — не про разрешимость вовсе, а про срок блокировки капитала. */
export const ATTR_CRITERIA = {
...
/* Полный список — для отчётов калибровки, которая измеряет всё. */
export const CRITERIA = { ...GATE_CRITERIA, ...ATTR_CRITERIA };
...
/* Термины, которые правила берут в кавычки. Иногда это имя поля в источнике
   («Arrivals of Ships»), но чаще — шаблонная обвязка вида resolve to "Yes".
   Обвязку отбрасываем, иначе поиск термина в источнике заведомо провалится
   у любого рынка, который не ссылается на именованный показатель. */
const BOILERPLATE = new Set(['yes', 'no', 'other', 'n/a', 'tbd', 'none']);
export function quotedTerms(text) {
  const out = new Set();
  for (const m of text.matchAll(/[«"“]([^»"”]{3,60})[»"”]/g)) {
    const t = m[1].trim();
    if (t.length >= 4 && !BOILERPLATE.has(t.toLowerCase())) out.add(t);
  }
  return [...out];
}
...
export function gateA(m, desc) {
  ...
  /* Отказ — только по воротам. Атрибуты уезжают в attrs и попадают в
     карточку рынка как предупреждения, а не как причина отсева. */
  const fails = Object.entries(checks)
    .filter(([k, ok]) => !ok && k in GATE_CRITERIA)
    .map(([k]) => GATE_CRITERIA[k] + (k === 'escape' ? ': ' + esc.join(', ') : ''));

  const attrs = Object.entries(checks)
    .filter(([k, ok]) => !ok && k in ATTR_CRITERIA)
    .map(([k]) => ({
      key: k,
      note: ATTR_CRITERIA[k] + (k === 'dates' ? `: ${ev.latest_date_in_rules} против ${ev.end_date}` : ''),
    }));

  return { pass: fails.length === 0, checks, fails, attrs, ev };
}

/* ---------- B. источник отдаёт названное ---------- */

/* Что один GET способен установить честно: источник существует и он
   машиночитаем. Интерфейс на JavaScript, который человек читает глазами,
   машиночитаемым источником не является — это и есть содержательная проверка.
 *
 * Чего он установить НЕ способен: «публикует ли источник названный
 * показатель». Для рынков, привязанных к ряду, имя поля в правилах есть и
 * совпадение термина осмысленно. Для событийных рынков имени поля нет вовсе,
 * и требовать совпадения означало бы повторить ошибку, из-за которой числовой
 * порог пришлось убирать из ворот: универсализировать частный случай.
 * Поэтому совпадение терминов — свидетельство на карточке, а не условие. */
export async function gateB(url, desc) {
  ...
  const fails = [];
  if (isShell) fails.push('источник — интерфейс на скриптах, а не машиночитаемые данные');

  /* Термины не отсеивают. Но если правила НАЗЫВАЮТ поле, а в источнике его
     нет, это стоит показать человеку и передать резолверу. */
  ev.terms_missing = terms.length > 0 && found.length === 0;
...
— отсутствие ЧИСЛОВОГО порога. Дискретное проверяемое событие с названным источником исполнимо ничуть не хуже числа: «назначен и приведён к присяге по данным официального вестника» — такой же механический критерий, как «значение ≥ 60». Требуй определённости, а не арифметики;
— отсутствие оговорки про недоступность данных, если исход не зависит от публикуемого ряда. Для событийных контрактов такая оговорка не нужна.

Требуй строго одного: чтобы в дату дедлайна процедура давала Yes или No без выбора между толкованиями.

Каждый input обязан нести дословную цитату ИЗ ТЕКСТА ПРАВИЛ. Блок метаданных цитировать нельзя: это служебные поля, а не условия контракта. Если подходящей цитаты в правилах нет — значит вход не описан правилами, и это повод для executable=false, а не для цитирования идентификатора.`;
